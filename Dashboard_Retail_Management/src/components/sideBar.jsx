      ...
      <nav className="flex-1 px-4 py-2 overflow-y-auto overflow-x-hidden custom-scrollbar">
        <div className="space-y-1 mt-4">
          {mainLinks.map((link) => (
            ...
            >
              {isActive(link.to) && (
                <span className="absolute left-0 w-1.5 h-6 bg-[#13786E] rounded-r-full" />
              )}
        ...
        {/* Administrator Section - Separated at the bottom of the list */}
        <div className="mt-8 pt-4 border-t border-teal-600/50">
          <Link
            to="/admin/administrator"
            className={`flex items-center gap-3 px-4 py-3 rounded-xl transition-all duration-200 group
              ${isActive("/admin/administrator") 
                ? "bg-white text-[#13786E] font-semibold" 
                : "hover:bg-white/10 text-teal-50"
              }`}
          >
            <span className={`${isActive("/admin/administrator") ? "text-[#13786E]" : "text-teal-300 group-hover:text-white"}`}>
              <ShieldCheck size={20} />
            </span>

...
import { 
  ...
  Store, Lock, ArrowRight, ArrowLeft, MessageCircle 
} from "lucide-react";
...
const SignUp = () => {
  ...
  const adminLoginMutation = useMutation({
    ...
    onError: (error) => {
      toast.error(error.response?.data?.message || "Admin access denied.");
    }
  });

  // --- STORE LOGIN MUTATION (New) ---
  const storeLoginMutation = useMutation({
    mutationFn: async (payload) => {
      // Backend route for store login
    ...
    onError: (error) => {
      // Backend se error message aayega: "Store not found" ya "Invalid Password"
  ...
  const handleSubmit = async (e) => {
    ...
    if (loginMode === "admin") {
      ...
      adminLoginMutation.mutate({ identifier: Formdata.identifier, password: Formdata.password });
    } else {
      // REAL STORE LOGIN CHECK
      if (!Formdata.storeName || !Formdata.storePassword) return toast.error("Please enter Store Name and Password");
  ...
  return (
    <div className="flex h-screen w-full overflow-hidden font-sans bg-white">
      {/* LEFT SIDE - Branding (Fixed and Compact) */}
        ...
        <div className="max-w-md">
          ...
          <div className="space-y-4">
            ...
            <div className="flex flex-col items-center gap-1">
               ...
               <p className="text-2xl font-black text-white bg-white/5 px-5 py-2 rounded-xl border border-white/10">03405542097</p>
            </div>
          </div>
          <div className="mt-10 lg:mt-16">
            <p className="text-[13px] text-teal-100/50 font-bold tracking-[3px] uppercase">A project of Apexiums Technologies</p>
          </div>
        </div>
      ...
      <div className="w-full lg:w-1/2 flex flex-col items-center justify-center p-6 md:p-10 relative">
        <div className="w-full max-w-sm">
          <div className="flex items-center gap-3 mb-6">
            <div className="bg-[#20b295] p-2.5 rounded-xl shadow-lg shadow-teal-500/20"><Building2 size={28} className="text-white" /></div>
            <div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4 text-left">
              ...
              <>
                <div className="space-y-1.5">
                  ...
                  <div className="relative">
                    ...
                    <input type="text" placeholder="e.g. Apexiums Mart" className="w-full pl-11 pr-4 py-3 rounded-xl border border-gray-200 focus:ring-2 focus:ring-[#20b295] outline-none text-sm font-medium transition-all" onChange={(e) => setFormdata({ ...Formdata, storeName: e.target.value })} value={Formdata.storeName}/>
                  </div>
                </div>
                <div className="space-y-1.5 relative">
            ...
            <button
              disabled={adminLoginMutation.isPending || storeLoginMutation.isPending}
              type="submit"
            ...
            >
              {(adminLoginMutation.isPending || storeLoginMutation.isPending) ? "Verifying..." : "Access Dashboard"}
              <ArrowRight size={16} />
            </button>
